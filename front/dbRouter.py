class defDBRouter(object):
    ...
    def db_for_read(self, model, **hints):
        ...
        if 'default' not in settings.DATABASES:
            return None
        if model._meta.app_label == 'front':
            return 'default'
        if model._meta.app_label == 'editorial':
            return 'default'
        return None
    ...
    def allow_syncdb(self, db, model):
        ...
        if 'default' not in settings.DATABASES:
            return None
        if db == 'default':
            return model._meta.app_label == 'front'
        elif model._meta.app_label == 'front':
            return True
        return None
